...
# test example:
# {
#   "business_id": "73p3vFFNt0SHbrA9Yv8x",
# }
...
URL = 'https://shakespeares.com/pizza/menu/'
...
def update(request):
    ...
    if request_body and 'business_id' in request_body:
        ...
        if len(menu_query_results) == 0: 
            ...
            raw_html = requests.post(URL, headers=DISTRIBUTION_HEADERS).content
            soup = BeautifulSoup(raw_html, 'html.parser')
            item_list = soup.find_all('li', class_='food-menu-item')
            counter = 0
            for item in item_list:
                menu_doc = menu_coll.document()
                menu_obj = {}
                name = item.find_all('h3', class_='food-menu-title')[0]
                price = item.find_all('span', class_='food-menu-price')[0]
                desc = item.find_all('p')[0]
                menu_obj['name'] = str(name.getText()).strip()
                menu_obj['description'] = str(desc.getText()).strip()
                menu_obj['price'] = str(price.getText()).strip()
