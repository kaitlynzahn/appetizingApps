...
# test example:
# {
#   "business_id": "s1G0m1kMFnMJNCajtSDZ",
# }
...
URL = 'https://www.flatbranch.com/menu/'
...
def update(request):
    ...
    if request_body and 'business_id' in request_body:
        ...
        if len(menu_query_results) == 0: 
            ...
            item_list = soup.find_all('div', class_='content twelve columns')
            counter = 0
            for item in item_list:
                menu_doc = menu_coll.document()
                menu_obj = {}
                name = item.find_all('h3', class_='name')[0]
                price = item.find_all('strong', class_='price')[0]
